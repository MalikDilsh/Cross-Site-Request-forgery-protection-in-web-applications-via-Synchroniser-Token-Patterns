<?php 
    ...
    //start a session with the users browser
    session_start();

    //setting up a cookie
    $sessionID = session_id(); 
    
    //storing session id with the cookie

    setcookie("session_id",$sessionID,time()+3600,"/","localhost",false,true);
    //cookie will beterminates after 1 hour or 3600 seconds and also this isHTTP only flag
...
<?php 
  ...
  //if in here cookie is ok request to the server and get CSRF token & store that token with inside hidden HTML DOM input tag ~ id=csToken
